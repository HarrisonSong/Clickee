        		...
        		success:function(output){
        			if(!$.isEmptyObject(output)){ 
        				...
        				$.each(output,function(index,element){
        					$('#roomSwitches').append(
        						'<div class = "span4 btn-warning" style = "margin-top:10px;margin-left:15px;background-color:#F89406;height:200px;-moz-border-radius: 5px;border-radius: 5px;">'+
        							'<h3 style = "margin-left:10px;">'+index+'</h3>'+
        						'</div>');
        					$.each(element,function(number,device){
        						if(number != 'no_ports'){
	        						$('#roomSwitches div.span4').last().append(
	        							'<dl class = "dl-horizontal" style = "margin-left:-60px;">'+
	        								'<dt style = "margin-bottom:10px;">'+device.name+'</dt>'+
        									'<dd style = "margin-bottom:10px;">'+
        										'<div class="btn-group" data-toggle="buttons-radio" id = "'+index+'Device'+number+'">'+
        											'<button class="btn btn-primary btn-small" onclick = "changeDeviceStatus(\''+index+'\','+number+',1)">ON</button>'+
        											'<button class="btn btn-primary btn-small" onclick = "changeDeviceStatus(\''+index+'\','+number+',0)">OFF</button>'+
        										'</div>'+
        									'</dd>'+
        								'</dl>');
			...
			$('.tab-pane.floor.active').append(
				'<div roomID = '+room_id+' name = "'+roomName+'" id = "roomArea" class="well well-small" style = "min-height:300px;">'+
			      ...
			      '<ul class="nav nav-tabs">'+
					'<li class = "active"><a href="#roomSwitches" data-toggle="tab">Devices</a></li>'+
					'<li><a onclick = "getEmployeeList('+room_id+')" href="#setting" data-toggle="tab">Setting</a></li>'+
				  '</ul>'+
				  '<div class = "tab-content">'+
				 	 '<div class="tab-pane active row-fluid" id="roomSwitches"></div>'+
			     	 '<div class = "tab-pane" style = "margin-left:20px;" id = "setting">'+
                ...
                <div class="container">
                  <a class="brand" href="#"><img class="pull-left" src="<?=ASSEST_URL?>desktop/img/ClickeeLogoSmall.png" center center no-repeat></a>
                        <ul class="nav">
                            ...
                            <li class = "active"><a href="<?php echo site_url("desktop/user/managerControlRoom");?>" style = "font-size:20px;text-transform: none;">Manage Room</a></li>
                        </ul>
			...
			<div class = "row-fluid">
				<div class = "span2">
					...
					<? foreach ($offices as $office) { ?>
					  <li style = "margin-left:-50px;"><a class = "officelist" name = '<?=$office["name"]?>' id = "office<?=$office["id"]?>" href = "<?=site_url("desktop/user/managerControlRoom/".$office["id"])?>"><i class = "icon-chevron-right pull-right"></i><?=$office["name"]?></a></li>
					<? } ?>
					</ul>
				</div>
